...
void main(List<String> arguments) {
  if (arguments.isEmpty) {
    print('❌ من فضلك أدخل اسم الصفحة مثل: mvvm_page_repo students_page');
    return;
  }

  final rawName = arguments.first;
  final pageName = _toSnakeCase(rawName); // students_page
  final className = _toPascalCase(rawName); // StudentsPage

  createMVVMWithRepoStructure(pageName, className);
}

void createMVVMWithRepoStructure(String pageName, String className) {
  ...
  final structure = {
    ...
    '$basePath/data/repository': '${pageName}_repository.dart',
    '$basePath/data/datasource': '${pageName}_datasource.dart',
  };
  ...
  print('✅ تم إنشاء صفحة "$pageName" مع repository و datasource بنجاح!');
}
...
''';
  } else if (fileName.endsWith('_repository.dart')) {
    return '''
abstract class ${className}Repository {
  // TODO: Define abstract methods
}
''';
  } else if (fileName.endsWith('_datasource.dart')) {
    return '''
class ${className}DataSource {
  // TODO: Connect to API or local DB
}
''';
  } else {
